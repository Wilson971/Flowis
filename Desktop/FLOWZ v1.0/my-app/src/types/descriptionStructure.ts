...
// ============================================
// SMART INTERNAL LINKING V2 - TYPES
// ============================================

// Style d'ancre pour les liens internes
export type AnchorStyle = 'semantic' | 'exact' | 'mixed';

// Configuration du maillage interne intelligent
export type LinkingOptions = {
    enabled: boolean;              // Activer le maillage intelligent
    max_product_links: number;     // 0-5, nombre max de liens produits
    max_category_links: number;    // 0-3, nombre max de liens catégories
    max_blog_links: number;        // 0-3, nombre max de liens articles blog
    anchor_style: AnchorStyle;     // Style d'ancre (sémantique, exact, mixte)
    similarity_threshold: number;  // 0.5-0.9, seuil de similarité minimum
    use_vector_search: boolean;    // Utiliser pgvector (si false, fallback sur tags)
};

// Configuration par défaut du maillage interne
export const DEFAULT_LINKING_OPTIONS: LinkingOptions = {
    enabled: true,
    max_product_links: 2,
    max_category_links: 1,
    max_blog_links: 1,
    anchor_style: 'semantic',
    similarity_threshold: 0.7,
    use_vector_search: true,
};

// Labels pour l'UI
export const ANCHOR_STYLE_LABELS: Record<AnchorStyle, string> = {
    semantic: 'Sémantique (ancres naturelles)',
    exact: 'Exact (titre du contenu)',
    mixed: 'Mixte (alternance)',
};

// Descriptions pour l'UI
export const ANCHOR_STYLE_DESCRIPTIONS: Record<AnchorStyle, string> = {
    semantic: 'Utilise des ancres générées par l\'IA pour un maillage naturel',
    exact: 'Utilise le titre exact du produit/article comme ancre',
    mixed: 'Alterne entre ancres sémantiques et titres exacts',
};
