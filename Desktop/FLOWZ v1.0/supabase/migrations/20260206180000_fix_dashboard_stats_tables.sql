-- Migration: Fix Dashboard Stats RPC - Correct table names
-- Description: Updates get_dashboard_stats to use blog_articles instead of blog_posts
-- Version: 20260206180000

-- ============================================================================
-- FIX get_dashboard_stats RPC FUNCTION
-- ============================================================================

-- Drop existing function and recreate with correct table names
...
BEGIN
    ...
    SELECT
        ...
        -- Compte total des articles de blog (FIXED: blog_articles instead of blog_posts)
        (SELECT count(*) FROM blog_articles b WHERE (p_store_id IS NULL OR b.store_id = p_store_id)),
        ...
        -- Produits non synchronisés (dirty fields)
        (SELECT count(*)
         FROM products p
         WHERE (p_store_id IS NULL OR p.store_id = p_store_id)
        ...
        -- Articles publiés (FIXED: blog_articles instead of blog_posts)
        (SELECT count(*)
         FROM blog_articles b
         WHERE (p_store_id IS NULL OR b.store_id = p_store_id)
         AND b.status = 'published'),

        -- Articles brouillons (FIXED: blog_articles instead of blog_posts)
        (SELECT count(*)
         FROM blog_articles b
         WHERE (p_store_id IS NULL OR b.store_id = p_store_id)
         AND b.status = 'draft');
...
'Returns dashboard statistics including products, categories, blog articles counts, SEO scores, and sync status. Fixed in v20260206180000 to use blog_articles table.';
