...
BEGIN
    ...
    SELECT
        ...
        -- Compte total des articles de blog (blog_articles table)
        ...
        -- Articles publiés (blog_articles table)
        (SELECT count(*)
         FROM blog_articles b
         WHERE (p_store_id IS NULL OR b.store_id = p_store_id)
         AND b.status = 'published'),

        -- Articles brouillons (blog_articles table)
        (SELECT count(*)
         FROM blog_articles b
         WHERE (p_store_id IS NULL OR b.store_id = p_store_id)
         AND b.status = 'draft');
